...
OUT=ROOT/'measurements/physical-reconstruction'
EXE=ROOT/'build/release/benchmarks/physical_reconstruction'
...
def main():
    ...
    cases=[(g,n,p,1.2,'gll','coupled',0) for g in ['quad','tri'] for p in [2,3,4] for n in ([4,8,16] if g=='quad' else [2,4,8])]
    cases += [(g,8,p,.4,'gll','coupled',0) for g in ['quad','tri'] for p in [2,3,4]]
    cases += [(g,4,p,0.,'gll','coupled',0) for g in ['quad','tri'] for p in [2,3,4]]
    cases += [(g,n,p,.7,'gll','separable',0) for g in ['quad','tri'] for p in [2,3,4] for n in ([4,8,16] if g=='quad' else [2,4,8])]
    cases += [(g,8,3,.3,'gll','separable',0) for g in ['quad','tri']]
    ...
    state={'description':'Matched DG input: recursive projection vs one physical Pq reconstruction, q=p,p+1,p+2; independent analytic-field oracle',
           'ranks':1,'threads_per_rank':1,'cpu_detection':'8 logical processors confirmed; one rank/thread chosen for small operator checks',
           'checks':[],'source_sha256':{str(p.relative_to(ROOT)):digest(p) for p in [ROOT/'benchmarks/physical_reconstruction.cpp',ROOT/'src/curved_geometry.hpp',EXE]},
           'mfem_config':(ROOT.parent/'mfem/config/config.mk').read_text(),'compute_seconds':205.75,'retained_attempt':'measurements/physical-reconstruction-normal-equations; mass-matrix solve failed polynomial reproduction control; charged 205.75 seconds', 'projection_solver':'weighted QR, twice-reorthogonalized modified Gram-Schmidt','planned_cases':len(cases)}
