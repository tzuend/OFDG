...
template<int D,int O> int Run(int n,bool curved){
 ...
 std::cout<<"dimension,n,curved,capacity,point,xi,eta,zeta,x,y,z,field,dx,dy,dz,order,method,value,input_error,geometry_residual,inverse_residual,initialization_seconds,evaluation_seconds\n"<<std::setprecision(17);
 for(int point=0;point<3;++point){IntegrationPoint ip;ip.Set3(points[point][0],points[point][1],points[point][2]);start=fe_mapping::Seconds();double residual=0,ar=0;
  ...
  for(int f=0;f<2;++f){auto u=high_order::Evaluate<D,O>(fv,coeff[f]),v=high_order::Evaluate<D,O>(av,coeff[f]);double elapsed=fe_mapping::Seconds()-start,input=std::abs(shape*original[f]-(f?polynomial(local):1));
   for(int i=1;i<L::N;++i){auto p=L::Powers()[i];for(int method=0;method<3;++method){double val=method==0?shape*recursive[f][i]:L::Factor(i)*(method==1?v.c[i]:u.c[i])/std::pow(h,L::Degree(i));
    std::cout<<D<<','<<n<<','<<curved<<','<<O<<','<<point<<','<<ip.x<<','<<ip.y<<','<<ip.z<<','<<x(0)<<','<<x(1)<<','<<(D==3?x(2):0)<<','<<(f?"quintic":"constant")<<','<<p[0]<<','<<p[1]<<','<<p[2]<<','<<L::Degree(i)<<','<<(method==0?"recursive":method==1?"direct":"fe_geometry")<<','<<val<<','<<input<<','<<gr<<','<<std::max(ar,residual)<<','<<initialization<<','<<elapsed<<'\n';}
   }
  }
 }
 return 0;
}
int main(int argc,char**argv){Mpi::Init(argc,argv);if(argc!=5){std::cerr<<"dimension n curved capacity\n";return 2;}int d=std::stoi(argv[1]),n=std::stoi(argv[2]),c=std::stoi(argv[3]),o=std::stoi(argv[4]);
 if(d==2&&o==5)return Run<2,5>(n,c);if(d==2&&o==10)return Run<2,10>(n,c);if(d==3&&o==5)return Run<3,5>(n,c);if(d==3&&o==10)return Run<3,10>(n,c);return 2;
}
