...
int main(int argc, char **argv)
{
    ...
    for (int e=0; e<space.GetNE(); ++e) {
        ...
        for (int enrichment=0; enrichment<3; ++enrichment) {
            ...
            // Weighted least squares avoids squaring the condition number of
            // the physical basis. Two-pass modified Gram-Schmidt constructs QR.
            const int ns=assembly.size();
            std::vector<Vector> orthogonal(nb,Vector(ns));
            for (int q=0;q<ns;++q) {
                Vector psi=pb.Evaluate(assembly[q].x);
                for (int j=0;j<nb;++j) { orthogonal[j](q)=std::sqrt(assembly[q].weight)*psi(j); }
            }
            DenseMatrix R(nb); R=0.;
            for (int j=0;j<nb;++j) {
                for (int pass=0;pass<2;++pass) {
                    for (int i=0;i<j;++i) {
                        double coefficient=orthogonal[i]*orthogonal[j];
                        R(i,j)+=coefficient;
                        orthogonal[j].Add(-coefficient,orthogonal[i]);
                    }
                }
                R(j,j)=orthogonal[j].Norml2();
                MFEM_VERIFY(R(j,j)>1e-14,"Physical reconstruction basis lost rank");
                orthogonal[j]/=R(j,j);
            }
            auto project=[&](int field,bool analytic,Vector &coefficients) {
                Vector values(ns); coefficients.SetSize(nb);
                for (int q=0;q<ns;++q) {
                    const auto &sample=assembly[q];
                    double value=analytic?Exact(field,sample.x,0,0,amplitude):sample.shape*U[field];
                    values(q)=std::sqrt(sample.weight)*value;
                }
                for (int j=0;j<nb;++j) { coefficients(j)=orthogonal[j]*values; }
                for (int j=nb-1;j>=0;--j) {
                    for (int i=j+1;i<nb;++i) { coefficients(j)-=R(j,i)*coefficients(i); }
                    coefficients(j)/=R(j,j);
                }
            };
            for (int f:fields) {
                auto &c=ed.reconstructed[enrichment][f]; project(f,false,c);
                ...
                if (enrichment==2) { project(f,true,oracle[f]); }
            }
        }
