...
struct FEElementData : direct::ElementData {std::unique_ptr<fe_mapping::GeometryField> field; std::array<std::unique_ptr<reused::Field>,2> rg,ru;};
int Run2D(int argc,char **argv) {
    ...
    reused::Plan reuse_plan(2,p>=3?3:2);
    ...
    reused::Operators geometry_ops(*first_iso->GetFE(),reuse_plan);
    ...
    for(int e=0;e<mesh.GetNE();++e){auto *iso=dynamic_cast<IsoparametricTransformation*>(mesh.GetElementTransformation(e));
        ...
        auto coordinates=reused::GeometryCoefficients(*iso);
        for(int c=0;c<2;++c)data[e].rg[c]=std::make_unique<reused::Field>(geometry_ops,coordinates,c);
    }
    ...
    for(int f:fields)for(auto word:words)for(int mode=0;mode<5;++mode){int nx=std::count(word.begin(),word.end(),'x');rows.push_back({f,nx,int(word.size())-nx,mode,word});}
    ...
    auto evaluate=[&](int e,const IntegrationPoint &ip,Vector &shape,std::array<direct::Jet,6>&values,std::array<direct::Jet,6>&fe_values,std::array<std::array<direct::Jet,6>,2>&reused_values) {
        ...
        for(int center=0;center<2;++center){
            reused::Geometry g(*ed.rg[center],ip,h);auto v=reused::Physical(g,*ed.ru[center],ip);
            reused_inverse_residual=std::max(reused_inverse_residual,g.inverse_residual);
            for(int d=0;d<2;++d)for(int a=0;a<2;++a)reused_geometry_residual=std::max(reused_geometry_residual,std::abs(g.K[0][d*2+a]-map->Jacobian()(d,a)/h));
            for(int f:fields){for(int k=0;k<reuse_plan.Size();++k){auto alpha=reuse_plan.indices[k];int i=direct::Index(alpha[0],alpha[1]);reused_values[center][f].c[i]=v(k,f)*std::pow(h,reused::Degree(alpha))/(direct::Factorial(alpha[0])*direct::Factorial(alpha[1]));}
                for(int d=0;d<2;++d){double expected=0;for(int j=0;j<fe->GetDof();++j)expected+=ds(j,d)*ed.original[f](j);reused_first_residual=std::max(reused_first_residual,std::abs(v(1+d,f)-expected)/std::max(1.,std::abs(expected)));}}
        }
    ...
    for(int e=0;e<space.GetNE();++e){double init_start=fe_mapping::Seconds();auto *fe=space.GetFE(e);auto *map=space.GetElementTransformation(e);int nd=fe->GetDof();auto &ed=data[e];qb=ofdg::detail::CurvedQuadratureOrder(p,*map);
        ...
        DenseMatrix solution_coefficients(nd,6);solution_coefficients=0.0;
        for(int f:fields)for(int j=0;j<nd;++j)solution_coefficients(j,f)=ed.original[f](j);
        for(int c=0;c<2;++c)ed.ru[c]=std::make_unique<reused::Field>(solution_ops,solution_coefficients,c);
        ...
        for(int q=0;q<ir.GetNPoints();++q){auto &ip=ir.IntPoint(q);std::array<direct::Jet,6> values,fe_values;std::array<std::array<direct::Jet,6>,2> reused_values;evaluate(e,ip,shape,values,fe_values,reused_values);map->Transform(ip,x);double w=ip.weight*map->Weight();minjac=std::min(minjac,map->Jacobian().Det());
            for(int f:fields){double d=shape*ed.original[f]-Exact(f,x,0,0,amplitude);input[f]+=w*d*d;}
            for(size_t i=0;i<rows.size();++i){auto &r=rows[i];double exact=Exact(r.field,x,r.nx,r.ny,amplitude),dv=derivative(values[r.field],r),v=r.mode>=3?derivative(reused_values[r.mode-3][r.field],r):r.mode==2?derivative(fe_values[r.field],r):(r.mode==1?dv:shape*ed.recursive[i]),err=v-exact;r.error+=w*err*err;r.truth+=w*exact*exact;r.linf=std::max(r.linf,std::abs(err));r.operatorerror+=w*(v-dv)*(v-dv);r.directnorm+=w*dv*dv;}
        }
    }
    for(int face=0;face<mesh.GetNumFaces();++face){auto *tr=mesh.GetFaceElementTransformations(face);if(!tr)continue;const auto &ir=IntRules.Get(tr->GetGeometryType(),qb+16+extra);
        for(int q=0;q<ir.GetNPoints();++q){auto &ip=ir.IntPoint(q);tr->SetAllIntPoints(&ip);Vector x;tr->Face->Transform(ip,x);double w=ip.weight*tr->Face->Weight();std::vector<double> left(rows.size());
            for(int side=0;side<2;++side){int e=side?tr->Elem2No:tr->Elem1No;if(e<0)continue;face_measure+=w;const auto &local=side?tr->GetElement2IntPoint():tr->GetElement1IntPoint();Vector shape(space.GetFE(e)->GetDof());std::array<direct::Jet,6> values,fe_values;std::array<std::array<direct::Jet,6>,2> reused_values;evaluate(e,local,shape,values,fe_values,reused_values);
                ...
                for(size_t i=0;i<rows.size();++i){auto &r=rows[i];double v=r.mode>=3?derivative(reused_values[r.mode-3][r.field],r):r.mode==2?derivative(fe_values[r.field],r):(r.mode==1?derivative(values[r.field],r):shape*data[e].recursive[i]),t=Exact(r.field,x,r.nx,r.ny,amplitude);r.face+=w*(v-t)*(v-t);r.facetruth+=w*t*t;if(side){r.jump+=w*(v-left[i])*(v-left[i]);r.jumpabs+=w*std::abs(v-left[i]);}else left[i]=v;}
                if(side)interior_measure+=w;
            }
        }
    }
    if(reused_inverse_residual>1e-10||reused_geometry_residual>1e-10||reused_first_residual>1e-8){std::cerr<<"Reuse control failed "<<reused_inverse_residual<<' '<<reused_geometry_residual<<' '<<reused_first_residual<<'\n';return 5;}
    ...
    for(const auto &r:rows)std::cout<<family<<','<<n<<','<<p<<','<<amplitude<<','<<basis<<','<<mapping<<','<<extra<<','<<names[r.field]<<','<<r.word<<','<<(r.mode==4?"reused_centered":r.mode==3?"reused":r.mode==2?"fe_geometry":r.mode==1?"direct":"recursive")<<','<<std::sqrt(r.error)<<','<<(r.truth>1e-24?std::sqrt(r.error/r.truth):NAN)<<','<<r.linf<<','<<std::sqrt(r.truth)<<','<<std::sqrt(r.face/face_measure)<<','<<(r.facetruth>1e-24?std::sqrt(r.face/r.facetruth):NAN)<<','<<std::sqrt(r.jump/interior_measure)<<','<<r.jumpabs/interior_measure<<','<<std::sqrt(input[r.field])<<','<<std::sqrt(r.operatorerror)<<','<<(r.directnorm>1e-24?std::sqrt(r.operatorerror/r.directnorm):NAN)<<','<<inverse_residual<<','<<representation_residual<<','<<first_derivative_residual<<','<<geometry_residual<<','<<minjac<<','<<space.GetNE()<<','<<geometry_derivative_residual<<','<<geometry_jacobian_residual<<','<<fe_inverse_residual<<','<<method_agreement<<','<<fe_first_residual<<','<<face_geometry_residual<<','<<initialization_seconds<<','<<evaluation_seconds<<','<<reused_inverse_residual<<','<<reused_first_residual<<','<<reused_geometry_residual<<'\n';
    return 0;
}
