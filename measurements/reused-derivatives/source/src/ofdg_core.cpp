...
public:
    ...
    mfem::DenseMatrix AssembleQl(int l, const mfem::DenseMatrix &M_k) const
    {
    ...
    const mfem::FiniteElement &fe)
{
    ...
    mfem::DenseMatrix vandermonde(ndof), dshape(ndof, dim);
    mfem::Vector shape(ndof);
    std::vector<mfem::DenseMatrix> values(dim, mfem::DenseMatrix(ndof));

    for (int q = 0; q < ndof; ++q) {
        const auto &ip = nodes.IntPoint(q);
        fe.CalcShape(ip, shape);
        fe.CalcDShape(ip, dshape);
        for (int i = 0; i < ndof; ++i) {
            vandermonde(q, i) = shape(i);
            for (int d = 0; d < dim; ++d) {
                values[d](q, i) = dshape(i, d);
            }
        }
    }

    vandermonde.Invert();
    std::vector<mfem::DenseMatrix> derivatives(dim, mfem::DenseMatrix(ndof));
    for (int d = 0; d < dim; ++d) {
        mfem::Mult(vandermonde, values[d], derivatives[d]);
    }
    return derivatives;
}
...
private:
    ...
    void BuildProjectionOperators(const OFDGProjection &projection)
    {
        ...
        // Positive orders need S_0,...,S_(order-1). Keep S_0 for
        // the existing constant-space stabilization path as well.
        const int remainder_count = std::max(1, order);
        S_l.reserve(remainder_count);
        for (int l = 0; l < remainder_count; ++l) {
            mfem::DenseMatrix S(ndof);
            mfem::DenseMatrix Q_l = projection.AssembleQl(l, M_k);
    ...
    {
        reference_derivatives = AssembleReferenceDerivatives(*finite_element);
    }
...
public:
    ...
    // Imported MPI faces are evaluated from their supplied transformations.
    void EvaluateDerivativeJumps(const mfem::DenseMatrix &derivatives1, const mfem::DenseMatrix &derivatives2,
...
void ApplyDerivativeGraph(const OFDGOperators &operators,
                          const std::vector<mfem::DenseMatrix> &derivatives,
                          OFDGDerivativeState &state)
{
    const auto &nodes = operators.DerivativeNodes();
    for (int node = 1; node < static_cast<int>(nodes.size()); ++node) {
        const auto &info = nodes[node];
...
private:
    ...
    void BuildState(const mfem::Vector &x, int element, OFDGDerivativeState &state) const
    {
        ...
        ApplyDerivativeGraph(operators, D, state);
    }
                                 ...
                                 OFDGDerivativeState &state) const
    {
        ...
        const auto &derivatives = operators.Curved() ? operators.ProjectedDerivatives() :
                                  affine_derivatives;
        ApplyDerivativeGraph(operators, derivatives, state);
    }
...
private:
    ...
    void AddFaceRates(int element, const mfem::DenseMatrix &jumps,
                      const mfem::Vector &scaling, double speed,
                      double face_height) const
    {
        const auto &operators = *mesh_data.Element(element).operators;
        for (int l = 0; l <= order; ++l) {
            const double prefactor = sensor_options.use_face_height
                ? operators.SensorCommon(l) * std::pow(face_height, l - 1)
                : mesh_data.SensorPrefactor(element, l);
            if (sensor_options.pool_components) {
                double pooled = 0.0;
                for (int c = 0; c < ncomp; ++c) {
                    if (!HasResolvedAmplitude(c, scaling)) { continue; }
                    pooled = std::max(pooled, prefactor * jumps(c, l) / scaling(c));
                }
                for (int c = 0; c < ncomp; ++c) {
                    sigma_elem[c](element, l) += speed * pooled;
                }
            } else {
                for (int c = 0; c < ncomp; ++c) {
                    if (!HasResolvedAmplitude(c, scaling)) { continue; }
                    const double inverse_scaling = 1.0 / scaling(c);
                    sigma_elem[c](element, l) +=
                        speed * prefactor * jumps(c, l) * inverse_scaling;
                }
            }
        }
    }
                              ...
                              const mfem::Array<bool> *active) const
    {
        ...
        for (int face = 0; face < mesh->GetNumFaces(); ++face) {
            ...
            face_evaluation.EvaluateDerivativeJumps(
                derivative_provider.Get(element1).all_coefficients,
                derivative_provider.Get(element2).all_coefficients, face, jumps,
                scratch.jump);
            ...
            AddFaceRates(element1, jumps, scaling, speed1,
                         face_evaluation.FaceHeight(face, true));
            AddFaceRates(element2, jumps, scaling, speed2,
                         face_evaluation.FaceHeight(face, false));
        }
    }
                               ...
                               const mfem::Array<bool> *active) const
    {
        ...
        for (int face = 0; face < parallel_mesh->GetNSharedFaces(); ++face) {
            ...
            AddFaceRates(local_element, jumps, scaling, local_speed, local_height);
        }
    ...
    void ComputeDampingRates(const mfem::Vector &state,
...
void StabilizationCore::ComputeStabilization(
    ...
    const mfem::Array<bool> *active) const
{
    implementation->ComputeStabilization(state, result, active);
}
...
void StabilizationCore::CompDecay(
    ...
    const mfem::Array<bool> *active) const
{
    implementation->CompDecay(state, result, decay_time, active);
}
