...
int main(int argc,char**argv){
 ...
 std::vector<Row> rows;for(int f=0;f<3;++f)for(int i=1;i<N;++i)for(int mode=0;mode<3;++mode)rows.push_back({f,i,mode});
 ...
 auto evaluate=[&](int e,const IntegrationPoint&ip,std::array<std::array<double,N>,3>&direct,std::array<std::array<double,N>,3>&feder,Vector&shape){
 ...
 for(int e=0;e<mesh.GetNE();++e){auto *fe=space.GetFE(e);auto *map=space.GetElementTransformation(e);const auto &ir=ofdg::detail::CurvedRule(fe->GetGeomType(),qb+16+extra);Vector shape,x;
  for(int q=0;q<ir.GetNPoints();++q){auto &ip=ir.IntPoint(q);std::array<std::array<double,N>,3> ad,fd;evaluate(e,ip,ad,fd,shape);map->Transform(ip,x);double w=ip.weight*map->Weight();
   for(int f=0;f<3;++f){double err=shape*data[e].original[f]-Exact(f,x,0,a,b);input[f]+=w*err*err;}
   auto refs=ExactValues(x,a,b);for(auto &r:rows){double ref=refs[r.field][r.index],dv=ad[r.field][r.index],v=r.mode==2?fd[r.field][r.index]:(r.mode==1?dv:shape*data[e].recursive[r.field][r.index]),err=v-ref;
