...
project-specific decision to gate OFDG with KXRCF.  This section states the
implemented affine choices explicitly; curved OEDG is not validated here.  In particular, the 2024 method is not a
...
whose state-dependent wave speeds can change. The unit-speed regression checks
scale/shift equivariance of attenuation in that fixed-speed setting.
...
assumptions~\cite{PengEtAl2024}. The preview supplies no new general invariance
claim for arbitrary nonlinear state transformations.
...
\texttt{step} choices are retained for cadence ablations.  The preview primarily compares DG, all-cell adapted OFDG, and
adapted OFDG--KXRCF. OEDG is included on selected affine shock cases. These
are fixed configurations, so observed differences include cadence as well as
sensor choices. Curved OEDG is not a validated paper-faithful extension and is
excluded from the curved comparison.
