...
int main(int argc, char *argv[])
{
   ...
   // Isolated visualization build: export full-precision initial/final states.
   auto save_frame = [&](const std::string &stage) {
      const std::string stem = profile_prefix + "-" + stage;
      std::ofstream mesh_file(stem + ".mesh"), state_file(stem + ".gf");
      mesh_file.precision(17); state_file.precision(17);
      mesh.Print(mesh_file); solution.Save(state_file);
   };
   save_frame("initial");
