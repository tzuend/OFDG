...
does not represent a smooth solution.  This superconvergence-based motivation
also explains the connection between KXRCF and the superconvergence measurements
used later in the numerical study~\cite{KrivodonovaEtAl2004,QiuShu2005}.
...
\end{equation}
Faces for which the normal velocity vanishes make no contribution.  This
...
Let $k$ be the polynomial degree, $h_K$ an element radius, $S_K$ a scale of
the solution on $K$, and $|\partial K^-|$ the measure of the inflow boundary.
...
\begin{equation}
  v_n=\tfrac12(u_1+u_2)\,\boldsymbol n,
\end{equation}
...
so no single scalar velocity represents every characteristic family.  OFDG and
OEDG therefore use the full one-sided radii
$|\boldsymbol v_i\cdot\boldsymbol n|+c_i$ for damping strength, whereas
...
and takes the largest center-to-vertex distance.  This is an exact
circumradius for the supported symmetric cells and a documented generalized
...
The first implementation recomputed radii, shapes, normals, and quadrature
weights on every call.  On the fixed benchmark with 9,216 quadrilateral
elements, degree three, four components, and 100 indicator calls, this required
2.8784 seconds.  The cached version requires 1.9436 seconds, a 32.5\% reduction.
Face integration remains the dominant phase at 1.5875 seconds, followed by
element scales at 0.3372 seconds and normalization at 0.0141 seconds.  Further
compression was not adopted because it would obscure the geometry and
ownership logic for a comparatively small additional benefit.

\subsection{Limitations}

The implementation assumes affine, full-dimensional MFEM elements and a fixed
mesh during the lifetime of the indicator.  Its radius generalization,
...
must be evaluated empirically and are therefore represented explicitly in the
